...
export default function AdminCollections() {
  ...
  return (
    <div className="p-8">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-gray-900">Collections & Produits</h1>
        <p className="text-gray-500 mt-1">Gérez les 6 collections de tenues et leurs produits</p>
      </div>
            ...
            {openId === c.id && col && (
              <div className="border-t border-gray-100 px-6 py-6 space-y-6">
                <div className="grid grid-cols-2 gap-4">
                  {[
                    { field: "titleEn" as const, label: "Titre (EN)", dir: "ltr" },
                    { field: "titleAr" as const, label: "Titre (AR)", dir: "rtl" },
                    { field: "subtitleEn" as const, label: "Sous-titre (EN)", dir: "ltr" },
                    { field: "subtitleAr" as const, label: "Sous-titre (AR)", dir: "rtl" },
                  ].map(f => (
                    <div key={f.field}>
                ...
                <div>
                  <p className="text-xs text-gray-500 mb-2">Image de couverture</p>
                  <ImageUpload
                ...
                <div>
                  <p className="text-sm font-semibold text-gray-800 mb-4">Produits ({prods.length})</p>
                      ...
                      <div key={p.id} className="border border-gray-200 rounded-xl p-4 space-y-3">
                        <div className="flex items-start gap-3">
                          ...
                          <div className="flex-1 grid grid-cols-2 gap-3">
                            {[
                              { field: "titleEn" as const, label: "Nom (EN)", dir: "ltr" },
                              { field: "titleAr" as const, label: "Nom (AR)", dir: "rtl" },
                            ].map(f => (
                              <div key={f.field}>
                                <label className="block text-xs text-gray-500 mb-1">{f.label}</label>
                                <input
                ...
                <div className="flex justify-end gap-3 pt-2 border-t border-gray-100">
                  {saved === c.id && <span className="text-green-600 text-sm font-medium self-center">✓ Sauvegardé !</span>}
                  <button
                  ...
                  >
                    {saving === c.id ? "Sauvegarde..." : "Sauvegarder cette collection"}
                  </button>
                </div>
              </div>
            )}
